"""
===============================================================================
Train CGAN with img, and t, as conditions
===============================================================================
"""
...
from configs.config_train_img_t import cfg
...
if __name__ == '__main__':
    ...
    for i_batch, batch in enumerate(train_dataloader):
        ...
        axs[0].set_title(batch['time_1'][0].numpy().item())
        axs[1].imshow(cfg['toPIL'](cfg['deNorm'](np.squeeze(batch['img_2'][0,:,:,:]))))
        axs[1].set_title(batch['time_2'][0].numpy().item())
        #plt.close(fig)
    
    
    #%% build a model        
    if cfg['use_model'] == 'gan':
        model = GANModel_img_t(dataModule.data_dims,cfg['g_e_net'],cfg['g_e_net_pretrained'],cfg['g_d_net'],cfg['g_d_net_pretrained'],cfg['dim_z'],cfg['dim_w'],cfg['dim_img'],cfg['dim_t'],cfg['z_fusion_type'],cfg['t_fusion_type'],cfg['data_time'],cfg['d_net'],cfg['d_net_pretrained'],cfg['d_transforms'],cfg['p_d_transforms'],cfg['lr'],cfg['losses_w'],cfg['final_actvn'])
    elif cfg['use_model'] == 'wgangp':
        model = WGANGPModel_img_t(dataModule.data_dims,cfg['g_e_net'],cfg['g_e_net_pretrained'],cfg['g_d_net'],cfg['g_d_net_pretrained'],cfg['dim_z'],cfg['dim_w'],cfg['dim_img'],cfg['dim_t'],cfg['z_fusion_type'],cfg['t_fusion_type'],cfg['data_time'],cfg['d_net'],cfg['d_net_pretrained'],cfg['d_transforms'],cfg['p_d_transforms'],cfg['lr'],cfg['losses_w'],cfg['final_actvn'])
    else:
    ...
    if cfg['use_model'] == 'gan':
        model = GANModel_img_t.load_from_checkpoint(ckpt_path)
    elif cfg['use_model'] == 'wgangp':
        model = WGANGPModel_img_t.load_from_checkpoint(ckpt_path)
