...
from models.discriminator_networks import Layer3Discriminator_img_t_if_cls, Layer5Discriminator_img_t_if_cls

from models.embedding import Embedding, TimeEmbedding


class WGANGPModel_img_t_if_cls(pl.LightningModule):
    def __init__(self, data_shape, g_e_net, g_e_net_pretrained, g_d_net, g_d_net_pretrained, dim_z, dim_w, dim_img, dim_t, dim_if, num_cls, z_fusion_type, t_fusion_type, data_time, d_net, d_net_pretrained, d_transforms, p_d_transforms, lr, losses_w, final_actvn, b1=0.0, b2=0.9, n_critic=5, lambda_gp=10):
        ...
        dim_if : int
            input dimension of influencing factors
        num_cls: int
            number of classes
        z_fusion_type : str
        ...
        if g_e_net=='res18':
            ...
            self.g_e = ResNet18EncoderCbnNoPool(self.c, self.dim_t*3)
        else:
            print('Wrong g_e_net')
        ...
        if g_d_net=='lightweight':
            ...
            if self.z_fusion_type == 'cat':
                self.g_d = ResNet18DecoderCbnNoPool(self.dim_t*3, cat_channels=self.dim_w)
            else: # None, 'add', 'wadd'
                self.g_d = ResNet18DecoderCbnNoPool(self.dim_t*3)
        else:
            print('Wrong g_d_net')
        print('Generator decoder: Total params:', sum(p.numel() for p in self.g_d.parameters()))
        
        # # Discriminator
        if d_net == 'layer3_img_t_if_cls':
            self.d = Layer3Discriminator_img_t_if_cls(self.c*2, self.dim_t, self.dim_if, self.num_cls, norm_layer=nn.InstanceNorm2d)
        elif d_net == 'layer5_img_t_if_cls':
            self.d = Layer5Discriminator_img_t_if_cls(self.c*2, self.dim_t, self.dim_if, self.num_cls, norm_layer=nn.InstanceNorm2d)
        else:
            print('Wrong d_net')
        ...
        # # influencing factors embedding
        self.if_emb_g = Embedding(self.dim_if, self.dim_t)
        
        if self.num_cls:
            self.cls_emb_g = nn.Embedding(self.num_cls, self.dim_t)
    ...
    def compute_gradient_penalty(self, real, fake, t_1, t_2, if_1, if_2, cls_1, cls_2):
        ...
        d_interpolates = self.d(interpolates,t_1,t_2,if_1,if_2,cls_1,cls_2)
    ...
    def configure_optimizers(self):
        ...
        g_parameters += list(self.if_emb_g.parameters())
        if self.use_noise_mapping:
            g_parameters += list(self.noise_mapping.parameters())
        if self.num_cls:
            g_parameters += list(self.cls_emb_g.parameters())
    ...
    def _forward(self, img_in, t_in, t_ref, if_in, if_ref, cls_in, cls_ref, z=None):
        if self.g_e_net=='res18_CBN' or self.g_e_net=='res18_CBN_noPool':
            x = self.g_e(img_in, torch.cat((self.t_emb_g(t_in),self.if_emb_g(if_in),self.cls_emb_g(cls_in)),dim=1))
        else:
            x = self.g_e(img_in)
        ...
        if self.g_d_net=='lightweight':
            # # embed times with pos_enc
            t_emb = torch.cat((self.t_emb_g(t_in),self.t_emb_g(t_ref)), dim=1)
            t_emb = t_emb.view(t_emb.shape[0], t_emb.shape[1], 1, 1)
            if_emb = torch.cat((self.if_emb_g(if_in),self.if_emb_g(if_ref)), dim=1)
            if_emb = if_emb.view(if_emb.shape[0], if_emb.shape[1], 1, 1)
            cls_emb = torch.cat((self.cls_emb_g(cls_in),self.cls_emb_g(cls_ref)), dim=1)
            cls_emb = cls_emb.view(cls_emb.shape[0], cls_emb.shape[1], 1, 1)
            x = self.g_d(torch.cat((x,t_emb,if_emb), dim=1))
        elif self.g_d_net=='lightweight_CBN' or self.g_d_net=='res18_CBN_noPool':
            x = self.g_d(x,torch.cat((self.t_emb_g(t_ref),self.if_emb_g(if_ref),self.cls_emb_g(cls_ref)),dim=1))
        return x
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        # # run forward pass
        logits_gen = self._forward(img_in,t_in,t_ref,if_in,if_ref,cls_in,cls_ref)
        # # activate output 
        ...
        if optimizer_idx == 0:
            # # run discriminator
            fake_validity = self.d(torch.cat((img_in,img_gen),dim=1),t_in,t_ref,if_in,if_ref,cls_in,cls_ref)
        ...
        # # train discriminator    
            ...
            fake_validity = self.d(torch.cat((img_in,img_gen),dim=1),t_in,t_ref,if_in,if_ref,cls_in,cls_ref)
            real_validity = self.d(torch.cat((img_in,img_ref),dim=1),t_in,t_ref,if_in,if_ref,cls_in,cls_ref)
            gradient_penalty = self.compute_gradient_penalty(torch.cat((img_in,img_ref),dim=1),torch.cat((img_in,img_gen),dim=1),t_in,t_ref,if_in,if_ref,cls_in,cls_ref)
    ...
    def forward(self, img_in=None, t_in=None, t_ref=None, if_in=None, if_ref=None, cls_in=None, cls_ref=None, z=None):
        ...
        # # generate if_in    
        if if_in == None:
            if_in = torch.randint(0, 1, (n_imgs,self.dim_if)).float().to(self.device)
        else:
            if_in = if_in.float().to(self.device)
        
        # # generate if_out    
        if if_ref == None:
            if_ref = torch.randint(0, 1, (n_imgs,self.dim_if)).float().to(self.device)
        else:
            if_ref = if_ref.float().to(self.device)
        
        # # generate cls_in    
        if cls_in == None:
            cls_in = torch.randint(0, 1, (n_imgs,)).to(self.device)
        else:
            cls_in = cls_in.to(self.device)
        
        # # generate cls_ref    
        if cls_ref == None:
            cls_ref = torch.randint(0, 1, (n_imgs,)).to(self.device)
        else:
            cls_ref = cls_ref.to(self.device)
            
        # run forward pass
        logits_gen = self._forward(img_in, t_in, t_ref, if_in, if_ref, cls_in, cls_ref, z=z)
        img_gen = self.actvn(logits_gen)
