'''
code reference: https://github.com/lucidrains/lightweight-gan/blob/main/lightweight_gan/lightweight_gan.py
'''
...
class LWGeneratorCBN(nn.Module):
    def __init__(
        self,
        image_size,
        n_aux,
        ...
        self.initial_conv = nn.Sequential(
            nn.ConvTranspose2d(latent_dim, latent_dim * 2, 4),
            ConditionalBatchNorm2d(latent_dim * 2, n_aux),
            nn.GLU(dim = 1)
        )
        ...
        for (res, (chan_in, chan_out)) in zip(self.res_layers, in_out_features):
            ...
            layer = nn.ModuleList([
                nn.Sequential(
                    ...
                    Blur(),
                    nn.Conv2d(chan_in, chan_out * 2, 3, padding = 1),
                    ConditionalBatchNorm2d(chan_out * 2, n_aux),
                    nn.GLU(dim = 1)
                ),
                sle,
                attn
    ...
    def forward(self, x, aux):
        # x = rearrange(x, 'b c -> b c () ()')
        x = self.initial_conv[0](x)
        x = self.initial_conv[1](x,aux)
        x = self.initial_conv[2](x)
        x = F.normalize(x, dim = 1)
