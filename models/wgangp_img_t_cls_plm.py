...
from models.discriminator_networks import Layer3Discriminator_img_t_cls, Layer5Discriminator_img_t_cls

from models.embedding import TimeEmbedding


class WGANGPModel_img_t_cls(pl.LightningModule):
    def __init__(self, data_shape, g_e_net, g_e_net_pretrained, g_d_net, g_d_net_pretrained, dim_z, dim_w, dim_img, dim_t, num_cls, z_fusion_type, t_fusion_type, data_time, d_net, d_net_pretrained, d_transforms, p_d_transforms, lr, losses_w, final_actvn, b1=0.0, b2=0.9, n_critic=5, lambda_gp=10):
        ...
        # # Discriminator
        if d_net == 'layer3_img_t_cls':
            self.d = Layer3Discriminator_img_t_cls(self.c*2, self.dim_t, self.num_cls, norm_layer=nn.InstanceNorm2d)
        elif d_net == 'layer5_img_t_cls':
            self.d = Layer5Discriminator_img_t_cls(self.c*2, self.dim_t, self.num_cls, norm_layer=nn.InstanceNorm2d)
        else:
            print('Wrong d_net')
    ...
    def compute_gradient_penalty(self, real, fake, t_1, t_2, cls_1, cls_2):
        ...
        d_interpolates = self.d(interpolates,t_1,t_2,cls_1,cls_2)
    ...
    def _forward(self, img_in, t_in, t_ref, cls_in, cls_ref, z=None):
        if self.g_e_net=='res18_CBN' or self.g_e_net=='res18_CBN_noPool':
            x = self.g_e(img_in, torch.cat((self.t_emb_g(t_in),self.cls_emb_g(cls_in)),dim=1))
        else:
            x = self.g_e(img_in)
        ...
        if self.g_d_net=='lightweight':
            ...
            x = self.g_d(x,torch.cat((self.t_emb_g(t_ref),self.cls_emb_g(cls_ref)),dim=1))
        return x
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        # # img_ref, t_ref, cls_ref
        img_ref = batch['img_2']
        t_ref = batch['time_2']
        cls_ref = batch['label']
                
        # # run forward pass
        logits_gen = self._forward(img_in,t_in,t_ref,cls_in,cls_ref)
        # # activate output 
        ...
        if optimizer_idx == 0:
            # # run discriminator
            fake_validity = self.d(torch.cat((img_in,img_gen),dim=1),t_in,t_ref,cls_in,cls_ref)
        ...
        # # train discriminator    
            ...
            fake_validity = self.d(torch.cat((img_in,img_gen),dim=1),t_in,t_ref,cls_in,cls_ref)
            real_validity = self.d(torch.cat((img_in,img_ref),dim=1),t_in,t_ref,cls_in,cls_ref)
            gradient_penalty = self.compute_gradient_penalty(torch.cat((img_in,img_ref),dim=1),torch.cat((img_in,img_gen),dim=1),t_in,t_ref,cls_in,cls_ref)
    ...
    def forward(self, img_in=None, t_in=None, t_ref=None, cls_in=None, cls_ref=None, z=None):
        ...
        # run forward pass
        logits_gen = self._forward(img_in, t_in, t_ref, cls_in, cls_ref, z=z)
        img_gen = self.actvn(logits_gen)
