...
class Embedding(nn.Module):
    def __init__(self, init_dim: int, emb_dim: int):
        """
        Only MLP embedding
        """
        super().__init__()
        ...
        self.mlp = nn.Sequential(
            nn.Linear(self.init_dim, self.emb_dim),
            nn.SiLU(),
    ...
    def forward(self, t: torch.Tensor):
        """
        Embeds an arbitrary vector t with an MLP.

        Args:
            t (torch.Tensor): Batch of inputs [bs,init_dim]

        Returns:
            t_emb (torch.Tensor): embeddings [bs,emb_dim]
        """
        t_emb = self.mlp(t)
...
if __name__ == "__main__":
    emb = TimeEmbedding(time_emb_dim=16, pos_emb_dim=64, max_len=100)
    time = torch.randint(0, 10, (4,))
    time_emb = emb.forward(time)
    
    timeA = torch.tensor(70)
    timeB = torch.tensor(60)
    
    time_embA = emb.forward(timeA)
    time_embB = emb.forward(timeB)
    
    # compute cosine similarity
    cosine = np.dot(torch.squeeze(time_embA).detach().numpy(),torch.squeeze(time_embB).detach().numpy())/(norm(torch.squeeze(time_embA).detach().numpy())*norm(torch.squeeze(time_embB).detach().numpy()))
