...
class GANModel_img_t(pl.LightningModule):
    def __init__(self, data_shape, g_e_net, g_e_net_pretrained, g_d_net, g_d_net_pretrained, dim_z, dim_w, dim_img, dim_t, z_fusion_type, t_fusion_type, data_time, d_net, d_net_pretrained, d_transforms, p_d_transforms, lr, losses_w, final_actvn):
        '''
        Parameters
        ----------
    ...
    def gan_criterion(self, img_gen, img_ref):
        loss_bce = self.loss_bce(img_gen, img_ref)
        return loss_bce
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            d_logits_fake = self.d(torch.cat((img_in,img_gen),dim=1),t_in,t_ref)
            
            # # compute gan loss
            g_loss = self.gan_criterion(self.d_actvn(d_logits_fake), torch.ones_like(d_logits_fake))
        ...
        # # train discriminator    
            ...
            d_logits_real = self.d(torch.cat((img_in,img_ref),dim=1),t_in,t_ref)
            d_loss_real = self.gan_criterion(self.d_actvn(d_logits_real), torch.ones_like(d_logits_real))   
            
            # discriminator fake loss
            d_logits_fake = self.d(torch.cat((img_in,img_gen),dim=1),t_in,t_ref)
            d_loss_fake = self.gan_criterion(self.d_actvn(d_logits_fake), torch.zeros_like(d_logits_fake))
            d_loss = (d_loss_real + d_loss_fake) / 2
            
            self.log("loss_d_real", d_loss_real)
            self.log("loss_d_fake", d_loss_fake)
