"""
===============================================================================
Train CGAN with img, t, if, and cls as conditions
===============================================================================
"""
...
from configs.config_train_img_t_if_cls import cfg
...
if __name__ == '__main__':
    ...
    for i_batch, batch in enumerate(train_dataloader):
        ...
        axs[0].set_title(str(batch['time_1'][0].numpy().item())+' '+str(np.round(batch['biomass_1'][0][0].numpy().item(),4))+' '+str(np.round(batch['biomass_1'][0][1].numpy().item(),4)))
        axs[1].imshow(cfg['toPIL'](cfg['deNorm'](np.squeeze(batch['img_2'][0,:,:,:]))))
        axs[1].set_title(str(batch['time_2'][0].numpy().item())+' '+str(np.round(batch['biomass_2'][0][0].numpy().item(),4))+' '+str(np.round(batch['biomass_2'][0][1].numpy().item(),4)))
        #plt.close(fig)
    
    
    #%% build a model        
    if cfg['use_model'] == 'wgangp':
        model = WGANGPModel_img_t_if_cls(dataModule.data_dims,cfg['g_e_net'],cfg['g_e_net_pretrained'],cfg['g_d_net'],cfg['g_d_net_pretrained'],cfg['dim_z'],cfg['dim_w'],cfg['dim_img'],cfg['dim_t'],cfg['dim_if'],cfg['num_cls'],cfg['z_fusion_type'],cfg['t_fusion_type'],cfg['data_time'],cfg['d_net'],cfg['d_net_pretrained'],cfg['d_transforms'],cfg['p_d_transforms'],cfg['lr'],cfg['losses_w'],cfg['final_actvn'])
    else:
    ...
    if cfg['use_model'] == 'wgangp':
        model = WGANGPModel_img_t_if_cls.load_from_checkpoint(ckpt_path)
